...
import { StatusPill, getButtonClass, CharacterAvatar, ProjectAvatar } from '@/components/ui'
...
export default async function ProjectsPage({
  ...
  const [
    ...
    projectList.length > 0
      ? supabase.from('hp_audits').select('id, project_id, created_at, input_signature:raw_data->>input_signature').in('project_id', projectIds)
            ...
            return (
              ...
              >
                ...
                <div className="flex items-start gap-4 mb-4">
                  {isLocked ? (
                    <div className="w-12 h-12 rounded-[var(--r)] flex items-center justify-center flex-shrink-0 bg-[var(--bg2)]">
                  ...
                  <div className="flex-1 min-w-0">
                    <Link href={`/projects/${project.id}`} className="group block rounded">
                      <div className="text-[18px] font-bold text-[var(--text)] mb-1 transition-colors group-hover:text-[var(--accent)]">{project.name || project.hp_url}</div>
                      <div className="text-[12px] text-[var(--text2)] overflow-hidden text-ellipsis whitespace-nowrap flex items-center gap-1 transition-colors group-hover:text-[var(--accent)]">
