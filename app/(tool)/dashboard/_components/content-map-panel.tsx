    ...
    <div className="bg-[var(--surface)] border border-[var(--border)] rounded-[var(--r-lg)] p-7">
      ...
      <div className="flex items-start justify-between mb-6">
        ...
        {classifications === null && (
          <div className="flex flex-col items-end gap-1">
            <DevAiLabel className="text-[13px] opacity-60">記事分析</DevAiLabel>
            <button
              type="button"
              onClick={runClassification}
              disabled={loading}
              aria-busy={loading}
              className="flex items-center gap-2 px-4 py-2 rounded-full text-base font-semibold text-white bg-[var(--accent)] hover:bg-[var(--accent-h)] transition-colors disabled:opacity-50 disabled:cursor-not-allowed cursor-pointer"
            >
              {loading ? (
                <>
      ...
      {classifications === null && !loading && (
        <div className="border border-dashed border-[var(--border)] rounded-xl p-8 flex flex-col items-center gap-3 text-center">
          <div className="text-base text-[var(--text2)] leading-relaxed">
      ...
      {loading && (
        <div role="status" aria-label="記事を分析中" className="flex flex-col items-center gap-3 py-10">
          <CharacterAvatar src={clausIcon} alt="クラウスのアイコン" emoji={clausEmoji ?? '🦉'} size={48} />
          <p className="text-base text-[var(--text2)]">記事の内容を読んでいます…</p>
        </div>
      )}
              ...
              <div className="grid mb-1" style={{ gridTemplateColumns: '72px repeat(4, 1fr)' }}>
                ...
                {EFFECTS.map((e) => (
                  <div key={e.key} className="text-center px-0.5">
                    <div className="text-[13px] font-semibold text-[var(--text)] leading-tight">{e.label}</div>
                    <div className="hidden sm:block text-[13px] text-[var(--text2)] leading-tight mt-0.5">{e.desc}</div>
                  </div>
                ))}
              </div>
                    ...
                    return (
                      ...
                      >
                        ...
                        <div className="relative z-10 text-center">
                          {isEmpty ? (
                            <div className="text-[13px] font-medium text-[#fb923c]">不足</div>
                          ) : (
                            <>
                              <div
