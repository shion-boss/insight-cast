...
export function BlogPreview({ latestPosts }: { latestPosts: Post[] }) {
  return (
    <section className="py-14 sm:py-[88px] bg-[var(--surface2)]">
      <div className="mx-auto max-w-[1160px] px-6 sm:px-8 lg:px-12">
        ...
        <div className="mt-8 flex flex-wrap items-center gap-2">
          <span className="text-[11px] font-semibold tracking-[0.12em] uppercase text-[var(--text2)] mr-1">テーマで探す</span>
        ...
        <div className="mt-6 flex flex-col divide-y divide-[var(--border)] rounded-[20px] border border-[var(--border)] bg-[var(--surface)] overflow-hidden">
          {latestPosts.map((post) => {
            ...
            return (
              <Link
                key={post.slug}
                href={`/blog/${post.slug}`}
                className="group flex items-start gap-4 px-5 py-5 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-[var(--accent)]/40"
              >
        ...
        <div className="mt-8">
          <Link href="/blog" className="border-[1.5px] border-[var(--border)] bg-[var(--surface)] text-[var(--text)] rounded-full px-6 py-3 text-base font-semibold hover:border-[var(--accent)] hover:text-[var(--accent)] transition-colors inline-flex items-center">
