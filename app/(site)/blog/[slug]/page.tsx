      ...
      <main id="main-content" className="relative z-10 mx-auto max-w-6xl px-6 py-12">
        ...
        <div className="mx-auto max-w-2xl">
            ...
            return (
              <div>
                ...
                <div className="mb-8 rounded-[var(--r-lg)] border border-[var(--border)]/80 bg-[rgba(255,253,249,0.94)] px-6 py-5 backdrop-blur-sm">
                  <p className="mb-3 text-base font-semibold text-[var(--text2)]">この記事でわかること</p>
                  <ul className="space-y-2">
                    {body.highlights.map((item, i) => (
                      <li
                        key={i}
                        className="relative pl-4 text-base leading-7 text-[var(--text2)] before:absolute before:left-0 before:top-3 before:h-1.5 before:w-1.5 before:rounded-full before:bg-[var(--accent)]"
                      >
                        {item}
                      </li>
                    ))}
                ...
                {interviewerChar && (
                  <div className="mb-8 flex items-start gap-3 rounded-[var(--r-lg)] border border-[var(--border)]/80 bg-[rgba(255,253,249,0.94)] px-5 py-4 backdrop-blur-sm">
                    ...
                    />
                    <div>
                      <p className="text-base font-semibold text-[var(--text2)]">
                        担当: {interviewerChar.name}（{interviewerChar.species}）
                      </p>
                      <p className="mt-1 text-base text-[var(--text2)]">{body.interviewerIntro}</p>
                    </div>
                  </div>
                )}
                  ...
                  {body.conversation.map((turn, i) => {
                    if (turn.role === 'owner') {
                      return (
                        <div key={i} className="flex gap-3">
                          <div className="flex-1 rounded-2xl border border-[var(--border)]/80 bg-[rgba(255,253,249,0.94)] px-5 py-4 text-base leading-7 text-[var(--text2)] backdrop-blur-sm">
                            {turn.text}
                          </div>
                    ...
                    return (
                      <div key={i} className="flex flex-row-reverse gap-3">
                        ...
                        <div className="flex-1 rounded-2xl border border-[var(--accent-l)] bg-[var(--accent-l)] px-5 py-4 text-base leading-7 text-[var(--text2)]">
                          {turn.text}
                        </div>
                      </div>
                    )
                ...
                <div className="rounded-[var(--r-lg)] border border-[var(--accent-l)] bg-[linear-gradient(145deg,rgba(255,247,230,0.98),rgba(255,253,249,0.95))] px-6 py-5">
                  <p className="mb-2 text-base font-semibold text-[var(--text2)]">取材のまとめ</p>
                  <p className="text-base leading-7 text-[var(--text2)]">{body.summary}</p>
                </div>
              </div>
            )
          })()}
          ...
          <div className="mt-6 rounded-[var(--r-lg)] border border-[var(--border)] bg-[var(--surface)] px-5 py-5">
            ...
            <p className="mt-2 text-base leading-[1.8] text-[var(--text2)]">
            ...
            <nav aria-label="前後の記事" className="mt-14 grid gap-3 sm:grid-cols-2">
              {prevPost ? (
                ...
                >
                  ...
                  <span className="mt-1 text-base font-semibold leading-snug text-[var(--text)] line-clamp-2 group-hover:text-[var(--accent)] transition-colors">{prevPost.title}</span>
                </Link>
              ) : <div />}
              {nextPost ? (
                <Link
                ...
                >
                  ...
                  <span className="mt-1 text-base font-semibold leading-snug text-[var(--text)] line-clamp-2 group-hover:text-[var(--accent)] transition-colors">{nextPost.title}</span>
                </Link>
              ) : <div />}
            </nav>
          )}
                  ...
                  >
                    ...
                    <div className="min-w-0 flex-1">
                      <div className="mb-1 flex items-center gap-2">
                        <span className="rounded-full px-2 py-0.5 text-[13px] font-bold" style={{ background: `${relColor}1a`, color: relColor }}>
