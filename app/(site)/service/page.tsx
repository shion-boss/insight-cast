...
const STEP_SUMMARY = [
  ...
  { step: 'STEP 03', title: '貼り付けるだけの記事を届ける', body: 'コピペするだけでブログに投稿できる状態で記事が届きます。' },
] as const
...
const STEP_DETAILS = [
  ...
  {
    step: '03',
    title: '貼り付けるだけの記事を届ける',
    paragraphs: [
      '取材が終わると、内容を整理した「取材メモ」と、ブログエディタにそのまま貼り付けられる「記事」が届きます。',
      '記事はタイトル・見出し・本文のブロック単位でコピーできます。自分のブログ投稿画面にパーツを貼るだけで、記事更新が完成します。',
    ],
    badges: ['取材メモ', 'ブロック単位でコピー', 'コピペで投稿'],
...
const DELIVERABLES = [
  ...
  {
    title: '記事',
    body: 'コピペするだけで投稿できる状態の記事。ブロック単位でコピーして、自分のブログにそのまま貼り付けられます。',
    charId: 'rain',
  },
] as const
