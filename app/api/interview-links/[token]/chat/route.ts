...
export async function POST(
  ...
  // 会話履歴取得（最新100件）。パス済み判定のために meta も取る
  ...
  // パス連発の防止: ユーザー回答無しの状態でAIに質問を再生成させ続けるとAPIコストが膨らむ。
  // 連続パスは 2 回まで許容（passStreak が 2 に達していたら次のパスは拒否）。
...
${
    isReturning
      ? `この回答者の方とは、これまでに${priorMeetingsCount}回お話しています。「はじめまして」と言わないこと。前回までのやりとりに今日のテーマと直接つながる話題があれば、それを1つだけ自然に触れてから本題に入ってください。つながりが薄い・思い当たらない場合は、過去には無理に触れず、再会の挨拶のあと今日のテーマからまっすぐ始めてください（関係しない過去テーマを冒頭に持ち出すと、今日の焦点がぼやけます）。`
