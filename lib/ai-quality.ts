...
- 調査結果や競合情報はヒントとして使い、そのまま読み上げない`,
  ...
  if (relationship === 'returning') {
    ...
    if (memoLines.length > 0) {
      // 関連する過去メモがある場合だけ「過去に触れて入る」を許可。
      parts.push(`【関係性: 再会（このユーザーとの取材は${priorMeetingsCount}回目）】
- 「はじめまして」「初めまして」と言わない
- 「またお話を聞きにきました」のような温度で入る
- 下の【前回までのやりとりの記憶】に、今日のテーマと直接つながる話題があれば、それを1つだけ自然に触れてから本題に入る
- つながりが薄い・該当なしと感じたら、過去には触れず、再会の挨拶だけ短く添えて今日のテーマからまっすぐ始める（関係しない過去テーマを冒頭に持ち出すと、今日の焦点がぼやけます）
...
この内容と重複する質問はしない。今日のテーマと近い場合は、そこから自然に深掘りする。`)
      ...
      // 過去メモはあるが今回のテーマに関連するものがない場合、
      // 過去に「触れる」よう強制しない。再会の温度だけ保つ。
      parts.push(`【関係性: 再会（このユーザーとの取材は${priorMeetingsCount}回目）】
- 「はじめまして」「初めまして」と言わない
- 再会の挨拶を一言だけ添えて、今日のテーマからまっすぐ始める
- **過去のテーマや前回の話題には触れない**（今日のテーマと直接つながる過去メモがないため、無理に触れると焦点がぼやけます）
- 「前回」「以前」「先日のお話」などの言葉も使わない`)
    }
  } else {
    parts.push(`【関係性: 初対面】
...
「${normalizePromptText(input.focusTheme, 80)}」を意識しつつも、テーマ名を繰り返すのではなく、事業者の実話や判断から自然に深掘りしてください。`)
  ...
  if (input.isGreeting) {
    if (relationship === 'returning') {
      const hasRelevantMemo = (input.pastInterviewMemos ?? []).some(
        (m) => Boolean(m.summary) || (m.themes && m.themes.length > 0) || Boolean(m.focusTheme),
      )
      if (hasRelevantMemo) {
        parts.push(`【最初の質問（再会）】
「はじめまして」「初めまして」と言わない。【前回までのやりとりの記憶】に今日のテーマと直接つながる話題があれば、それを1つだけ自然に触れてから今日のテーマに入る。つながりが薄ければ過去には触れず、再会の挨拶を一言添えて今日のテーマから始める。最後に答えやすい1問を出してください。`)
      } else {
        parts.push(`【最初の質問（再会）】
「はじめまして」「初めまして」と言わない。**今日のテーマに関連する過去のやりとりは記録されていません**。「前回」「以前」「先日のお話」など過去を匂わせる言い回しは使わず、再会の挨拶だけ短く添えて、今日のテーマに合った答えやすい1問を出してください。`)
      }
    } else {
