...
/**
 * 日本語のキーワード抽出。
 * 助詞 + 記号 + 句読点で細かく分割し、2 文字以上のセグメントを抽出する。
 *
 * 旧実装はスペース区切りのみだったため「カフェの差別化」が 1 セグメントになり、
 * 過去メモに「カフェ」だけしか出てこない場合にマッチしなかった。
 * 助詞（の・を・に等）でも区切ることで、意味のある語の単位でマッチング可能にする。
 *
 * bigram は採用しない（「カフ」がノイズとして「カフカ」「カフ」始まりの別語にマッチして
 * 関係しない過去メモが「関連」と判定される副作用が出るため）。
 */
function extractKeywords(text: string): string[] {
  const segments = text
    .replace(/[、。・,.\s（）()「」『』【】\[\]:：]+/g, ' ')
    // 主要な助詞で区切る（の / を / に / は / が / で / と / へ / も / や / から / まで / より / だけ）
    .replace(/(?:から|まで|より|だけ|について|として|による)/g, ' ')
    .replace(/[をにはがでとへもやの]/g, ' ')
    .split(' ')
    .map((t) => t.trim())
    .filter((t) => t.length >= 2)
