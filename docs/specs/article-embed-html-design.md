...
**ラッパー**
- 背景: なし（透明。埋め込み先に馴染む）
- `max-width: 800px`, `margin: 0 auto`, `padding: 28px 4px`
...
**テキスト改行ルール**
- `。！？` の直後に `<br>` を挿入して文ごとに改行する
- ただし以下では `<br>` を入れない:
  - 直後の文字が閉じ括弧（`」` `』` `）` `)`）の場合（例: `！」` `。)` の間で改行しない）
  - その `。！？` が `「」` / `『』` / `()` / `（）` の内側にある場合（引用文・補足の中の文区切りでは改行しない）

**質問バブル（右寄せ・アイコンなし）**
- `display: flex`, `justify-content: flex-end`, `margin-bottom: 16px`
- バブル: `background: #ede8e0`, `border-radius: 16px 4px 16px 16px`
- `padding: 4px 12px`, `max-width: 60%`
- `font-size: 15px`, `line-height: 1.85`, `color: #3d2b1f`

**回答バブル（左寄せ・イニシャルバッジあり）**
- `display: flex`, `align-items: flex-start`, `gap: 4px`, `margin-bottom: 16px`
- バッジ: `width: 32px`, `height: 32px`, `border-radius: 50%`, `background: #d4a26a`, 白文字, `font-size: 13px`, `font-weight: 700`, `margin-top: -8px`（吹き出しより上に飛び出す）
