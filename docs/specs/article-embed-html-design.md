...
Q&A形式の会話記事。Cast Talkライクな左右交互バブルレイアウト。
...
| `created_at` | 公開日 |
| `interviewer_id` | キャストID |
| `interviewer_name` | キャスト名 |
| `client_name` | 事業者名（取材先名） |
...
- 背景: `#fdf7f0`
- キャストバブル（左）: `background: #fff8f0`, `border: 1px solid #e2d5c3`, `border-radius: 4px 16px 16px 16px`, `padding: 12px 16px`, `max-width: 75%`
- 事業者バブル（右）: `background: #f0f4ff`, `border: 1px solid #d4d9f0`, `border-radius: 16px 4px 16px 16px`, `padding: 12px 16px`, `max-width: 75%`
- イニシャルバッジ: `width: 32px`, `height: 32px`, `border-radius: 50%`, キャストカラー背景, 白文字, `font-size: 12px`, `font-weight: 700`
- 会話以外の本文（導入文など）: `font-size: 14px`, `line-height: 1.8`, `color: #7a6555`, `margin-bottom: 24px`
