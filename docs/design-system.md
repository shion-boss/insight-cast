...
ブランドオレンジ（`--primary`）はあくまで CTA・強調・focus 用。**テキストリンクには別の青系ロール `--link` を使う**ことで、

- 「クリックできる文中要素」と「ブランド強調」が見た目で区別できる
- オレンジ + 青の役割分離で、CTA の主張が薄まらない
- リンクとしての可読性（青＝リンク）の Web 慣例と整合する

| Role | Value | 用途 |
|---|---|---|
| `--link` | `#1d4ed8` | 本文中のテキストリンク（blue-700）。`--surface` 上で 6.94:1（AA） |
| `--link-hover` | `#1e40af` | テキストリンクの hover（blue-800）。`--surface` 上で 8.59:1（AAA） |

#### 使い分け
- 本文・記事・FAQ・規約の中の `<a>` / `<Link>` → `--link`
- LP のヒーローCTA・料金ページの「申し込む」等の **ボタン形リンク** → 引き続き `--primary`（背景色として）
- ナビゲーション・メニューのリンク → コンテキストに応じて。装飾優先なら `--on-surface` で hover で `--link` または `--primary` に。

#### 実装ルール
- 新規実装で `<a>` / `<Link>` を本文中に置くときは **クラスで色を上書きしない**。`globals.css` のデフォルト（`a:where(...)` セレクタ）で自動的に `--link` が当たる
- 既存コードで `text-[var(--accent)]` `text-[var(--primary)]` がリンク用途に使われている箇所は、ページタッチ時に `text-[var(--link)]` に置換する
- ボタン形リンク（`bg-[var(--primary)]` 等）は `text-[var(--on-primary)]`（白）のまま。`--link` には変えない
