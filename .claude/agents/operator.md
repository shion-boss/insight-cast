...
**オペレーター は全社員の「気づき」を集約して週次でパターン化する担当。**

- 週次レビュー（`ops/weekly-review.md`）に以下のセクションを必ず含める:
  ```
  ### 今週の指摘パターン集計
  | カテゴリ | 件数 | 初出/再発 | ルール化済みか |
  |---|---|---|---|
  | 導線パラメータ欠落 | 2 | 再発 | ✅ CLAUDE.md #8 |
  | コピー文脈不一致 | 1 | 初出 | 🔲 要提案 |
  ```
- 「再発」かつ「ルール化済み」なのに繰り返す場合 → ルールの書き方が不十分。ディレクター に改善依頼
- 「初出」が3週連続で同種なら → パターン化してCLAUDE.mdに追記提案
- 各社員から届いた「落とし穴メモ」を週次でまとめ、CLAUDE.md 更新候補リストを作る
