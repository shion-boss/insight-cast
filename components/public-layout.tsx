            ...
            <aside className="hidden lg:block mt-6">
              <div className="sticky top-28 rounded-[var(--r-lg)] border border-[var(--border)] bg-[var(--surface)] p-5">
                ...
                <div className="mt-4 space-y-3 text-sm text-[var(--text2)]">
                  <Link href="/privacy" className="block rounded transition-colors hover:text-[var(--link)]">プライバシーポリシー</Link>
                  <Link href="/terms" className="block rounded transition-colors hover:text-[var(--link)]">利用規約</Link>
                  <Link href="/tokushoho" className="block rounded transition-colors hover:text-[var(--link)]">特定商取引法に基づく表記</Link>
                </div>
              </div>
            </aside>
